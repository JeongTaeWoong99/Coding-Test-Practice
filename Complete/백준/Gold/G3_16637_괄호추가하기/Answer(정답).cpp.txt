...
int main()
{
    ...
    cin >> originS; // 수식 문자열 (예: "3+8*7-9*2")

    // 수식 파싱 : 숫자와 연산자 분리
    for(int i = 0; i < N; i++)
    {
        if(i % 2 == 0) numVec.push_back(originS[i] - '0'); // 짝수 인덱스 → 숫자 저장
        else           operVec.push_back(originS[i]);         // 홀수 인덱스 → 연산자 저장
    }
