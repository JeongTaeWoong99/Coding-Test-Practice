...
`start ~ end` 범위에서 `square`의 첫 배수를 찾으려면:
- `start % square`를 계산하면______→ start가 square의 몇 번째 위치에 있는지 알 수 있음
- `square - (start % square)`를 계산하면_→ 다음 배수까지 몇 칸 남았는지
- 단, `start`가 이미 square의 배수면__→ `% square`로 0으로 만들어줌
